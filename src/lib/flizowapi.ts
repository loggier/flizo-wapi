...
export async function createInstance(instanceName: string, token: string, number?: string): Promise<FlizoWapiResponse> {
  try {
    const payload: any = {
      instanceName,
      token,
...
export async function logoutInstance(instanceName: string, instanceApiKey: string): Promise<FlizoWapiResponse> {
  try {
...
export async function deleteInstance(instanceName: string, instanceApiKey: string): Promise<FlizoWapiResponse> {
  try {
...
export async function sendMessage(instanceName: string, apiKey: string, payload: { number: string; text: string }): Promise<FlizoWapiResponse> {
  try {
...
export async function fetchInstances(): Promise<FlizoWapiResponse> {
  try {
